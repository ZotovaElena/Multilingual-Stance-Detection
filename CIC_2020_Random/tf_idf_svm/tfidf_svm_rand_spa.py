...
#load datasets 
tweets_t = pd.read_csv('train_user_exp_rand.csv', dtype={'id_str': 'str'}, sep='\t', encoding="utf-8")
tweets_t = tweets_t.fillna('')

tweets_val = pd.read_csv('val_user_exp_rand.csv', dtype={'id_str': 'str'}, sep='\t', encoding="utf-8")
tweets_val = tweets_val.fillna('')

tweets_test = pd.read_csv('test_user_exp_rand.csv', dtype={'id_str': 'str'}, sep='\t', encoding="utf-8")
...
clf = Pipeline(
    [  ("filter", SelectKBest(mutual_info_classif, k=num_features)),
       ("classification", svm.SVC(kernel="rbf", gamma=0.1, C=10)),
    ]
)
...
file_name = 'SVM_CV_report_exp_rand.csv'
...
tweets_train.to_csv('SVM_CV_predicted_exp_rand.csv', encoding='utf-8', index=False)
df1 = tweets_train.loc[tweets_train['predicted'] != tweets_train['true']]
wrong_prediction = df1[['TWEET_x', 'LEMMA_CLEAN', 'LABEL', 'id_str', 'predicted', 'true']]
wrong_prediction.to_csv('SVM_CV_wrong_predicrion_exp_rand.csv', encoding='utf-8', index=False)
...
file_name = "SVM_test_report_exp_rand.csv"
...
tweets_test.to_csv("SVM_test_predicted_exp_rand.csv", sep='\t', encoding='utf-8', index=False)
df2 = tweets_test.loc[tweets_test['predicted'] != tweets_test['true']]
wrong_prediction_test = df2[['TWEET_x', 'LEMMA_CLEAN', 'LABEL', 'id_str', 'predicted', 'true']]
wrong_prediction_test.to_csv("SVM_test_wrong_prediction_exp_rand.csv", encoding='utf-8', index=False)
