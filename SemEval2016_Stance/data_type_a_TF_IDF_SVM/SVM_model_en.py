...
file_name = str(num_features)+'grid_result.csv'
grid_result.to_csv(os.path.join(path, file_name), encoding='utf-8', index=False)
...
print('C ', C)
print('gamma ', gamma)
df_grid_first.to_csv(os.path.join(path, 'best_param_'+str(num_features)+'.csv'), encoding='utf-8', index=False)
...
cv_results = cross_validate(clf, X_train, y_train, cv=10, n_jobs=-1, 
                            scoring='f1_macro')
...
tweets_train.to_csv(os.path.join(path, 'SVM_CV_predicted.csv'), encoding='utf-8', index=False)
...
tweets_test.to_csv(os.path.join(path, "SVM_test_predicted.csv"), sep='\t', encoding='utf-8', index=False)
...
# wrong_prediction_test.to_csv(os.path.join(path, "SVM_test_wrong_prediction.csv"), encoding='utf-8', index=False)
